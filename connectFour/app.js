...
// The rotation from player 1 to player 2 
                ...
                if (squares[index + 7].classList.contains('taken')) {
                    if (currentPlayer === 1) {
                        ...
                        squares[index].classList.add('player-one');
                                // player one and player 2 onclick function
                        currentPlayer = 2;
                    ...
                    // message that alerts the user that they cant go here
                    alert('You cant add chip with no leading player chip.');
                }
            }
        })(i)
    }
    ...
    function checkboard() {
        ...
        for (let y = 0; y < winningArrays.length; y++) {
            const checkSqaures = [
                squares[winningArrays[y][0]],
                squares[winningArrays[y][1]],
                squares[winningArrays[y][2]],
                squares[winningArrays[y][3]]
                // how many needed to win 
            ];
            let hasPlayerWon = false;
            let hasConfirmed = false;

            let player1 = checkSqaures.filter((sq) => sq.classList.contains('player-one'));
            let player2 = checkSqaures.filter((sq) => sq.classList.contains('player-two'));
